...
cvx_begin
    ...
    for k = 1:para.K
        ...
        % NOMA constraint with SIC (ascending order): user k cancels weaker users 1..k-1,
        % experiences interference from stronger users k+1..K
        interference = 0;
        for i = k+1:para.K
            interference = interference + quad_form(conj(hk), F(:,:,i));
        end
        quad_form(conj(hk), Fk) >= (2^para.Rmin - 1) * (interference + 1);
    end
